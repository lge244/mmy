...
//            }
			...
			if (empty($type)) {
				...
				$user = $_SESSION['user'];
				if (count($user) <= 0) {
					$user = pdo_getall('users', array('jobid' => 2), array('uid'));
					$_SESSION['user'] = $user;
				}
				...
				$data = array('uniacid' => $_W['uniacid'], 'cid' => $id, 'mobile' => $mobile, 'nickname' => $nickname, 'openid' => $openid, 'pwd' => md5($pwd . $salt), 'salt' => $salt, 'createtime' => time(), 'mobileverify' => 1, 'comefrom' => 'mobile');
			} else {
				if (empty($member)) {
					show_json(0, '此手机号未注册');
				}
			...
			if (empty($member)) {
				...
				$member = pdo_get('ewei_shop_member', array('id' => $uid), array('invite'));
				$invite = $member['invite'] + 1;
				pdo_update('ewei_shop_member', array('invite' => $invite), array('id' => $uid));
