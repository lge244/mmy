...
class Index_EweiShopV2Page extends MobilePage
{
	public function main()
	{
		...
		$m_order = pdo_getall('ewei_shop_order', array('status' => 3, 'paytime >' => strtotime($time[0]), 'paytime <=' => strtotime($time[1])));
		$count = 0;
		foreach ($m_order as $k => $v) {
			$count += $v['price'] - $v['dispatchprice'] - $v['taxes'];
		}
		$proportion = pdo_get('shop_shareholder_rule');
		$m_money = ($count * $proportion['shareholder_ratio']) / 100;
