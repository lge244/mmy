...
class List_EweiShopV2Page extends WebPage
{
    ...
    private function order_list($role_id, $shop_id = [], $order_status = 0)
    {
        ...
        // $list = pdo_fetchall("SELECT *,a.id FROM `ims_ewei_shop_purchase_order` `a`
        //						LEFT JOIN `ims_ewei_shop_goods` `b` ON `a`.`good_id`=`b`.`id`
        //						where `order_status` = " . $order_status . " ORDER BY `a`.`order_status` ASC");
        $sum_money = null;
        foreach ($order as &$v) {
            $v['good_info'] = pdo_get('ewei_shop_goods', ['id' => $v['good_id']]);
            $sum_money += $v['good_info']['marketprice'] * $v['number'];
        }
        return ['list' => $order, 'sum_money' => $sum_money, 'total' => count($order)];
    }
    ...
    public function purchase()
    {
        ...
        $store_id = pdo_getall('ewei_shop_store', ['status' => 1]);
        $order_status = $_GPC['order_status'] ? $_GPC['order_status'] : 0;
        if ($role_id == 4) $order_status = 1;
        $list = $this->order_list($role_id, $shop_id, $order_status);
        $total = $list['total'];
        ...
        if (isset($_SERVER['REQUEST_METHOD']) && strtoupper($_SERVER['REQUEST_METHOD']) == 'POST') {
            ...
            $list = $this->searchList($where['store_id'], $where['order_status']);
            $list = $list['list'];
            $sum_money = $list['sum_money'];
            $total = $list['total'];
            $search = true;
    ...
    public function searchList($shop_id, $order_status)
    {
        global $_W;
        global $_GPC;
        /*	$role_id = $_W['user']['jobid'];
        ...
        if (isset($where['store_id']) && isset($where['order_status'])) {
            $list = pdo_fetchall("
					SELECT *,a.id FROM `ims_ewei_shop_purchase_order` `a` 
					LEFT JOIN `ims_ewei_shop_goods` `b` ON `a`.`good_id`=`b`.`id` 
					where `order_status` = " . $where['order_status'] . " AND `shop_id` = ". $where['store_id'] ." ORDER BY `a`.`order_status` ASC
				");
        }
        if (isset($where['store_id']) && !isset($where['order_status'])) {
            $list = pdo_fetchall("
					SELECT *,a.id FROM `ims_ewei_shop_purchase_order` `a` 
					LEFT JOIN `ims_ewei_shop_goods` `b` ON `a`.`good_id`=`b`.`id` 
					where `shop_id` = ". $where['store_id'] ." ORDER BY `a`.`order_status` ASC
				");
        }
        if (!isset($where['store_id']) && isset($where['order_status'])) {
            $list = pdo_fetchall("
					SELECT *,a.id FROM `ims_ewei_shop_purchase_order` `a` 
					LEFT JOIN `ims_ewei_shop_goods` `b` ON `a`.`good_id`=`b`.`id` 
					where `order_status` = " . $where['order_status'] . " ORDER BY `a`.`order_status` ASC
				");
        }
        $total = count($list);
        $sum_money = null;
        foreach ($list as &$v) {
            $sum_money += $v['marketprice'] * $v['number'];
        }
        return ['list' => $list, 'sum_money' => $sum_money, 'total' => $total];
        // include $this->template('order/purchase');
    }
    ...
    public function updatePurchase()
    {
        ...
        // 当前获得积分
        $integral = (number_format(str_replace(',', '', $money)) * -100) * ($good['proportion'] / 100);
        // 当前商品是否存在
    ...
    public function allUpdatePurchase()
    {
        global $_GPC;
        global $_W;
        $role_id = $_W['user']['jobid'];
        ...
        foreach ($data as $key => &$val) {
            ...
            unset($val[3]);
            unset($val[4]);
            $val['good'] = pdo_get('ewei_shop_goods', ['id' => $val['good_id']]);
            unset($val['good']['id']);
            $val['$store'] = pdo_get('ewei_shop_store', ['id' => $val['shop_id']], 'uid');
            $v['user_integral'] = pdo_getcolumn('users', ['uid' => $val['store']['uid']], 'integral');
            $val['integral'] = (number_format($val['money']) * -100) * ($val['good']['proportion'] / 100);
            $v['good']['shop_id'] = $v['shop_id'];
            $v['good']['good_id'] = $v['good_id'];
            $v['good']['total'] = $v['number'];
        }
        // dump($data);die;
        // 当前为财务
        if ($role_id == 3) {
            foreach ($data as $v) {
                ...
                if (!$res) exit(json_encode(['status' => 0, 'msg' => 'id为' . $v['order_id'] . '订单确认收款失败']));
            }
            exit(json_encode(['status' => 1, 'msg' => '确认收款成功']));
        }
        // 当前为仓管
        if ($role_id == 4) {
            foreach ($data as $v) {
                $order = pdo_get('ewei_shop_purchase_order', ['id' => $v['order_id']]);
                if ($order['purchase_type'] == 1) {
                    $record_integral = pdo_insert('ewei_users_integral', [
                        'integralnum' => $v['integral'],
                        'type'        => 1,
                        'mid'         => $v['store']['uid'],
                        'usetime'     => time()
                    ]);
                    $update_user = pdo_update('users',
                        ['integral' => $v['integral'] + $v['user_integral']],
                        ['uid' => $v['store']['uid']
                        ]);
                }
                $res = pdo_update('ewei_shop_purchase_order',
                ...
                $purchase = pdo_get('ewei_shop_purchase', ['shop_id' => $v['shop_id'], 'good_id' => $v['good_id']]);
                if (!$purchase) {
                    $v['good']['good_id'] = $v['good_id'];
                    $v['good']['shop_id'] = $v['shop_id'];
                    $v['good']['stock'] = $v['number'];
                    $purchase_res = pdo_insert('ewei_shop_purchase', $v['good']);
                } else {
                    $purchase_res = pdo_update('ewei_shop_purchase', [
                        'stock' => $purchase['stock'] + $v['number']
                    ], ['id' => $purchase['id']]);
                }
                if (!$res || !$purchase_res) {
                    exit(json_encode(['status' => 0, 'msg' => 'id为' . $v['order_id'] . '确认发货失败']));
                }
            }
            exit(json_encode(['status' => 1, 'msg' => '确认发货成功']));
        }
        ...
        if ($role_id == 0) {
            foreach ($data as $v) {
                $order_status = pdo_get('ewei_shop_purchase_order', ['id' => $v['order_id']], ['order_status','purchase_type']);
                if ($order_status['order_status'] == 0) {
                    $res = pdo_update('ewei_shop_purchase_order',
                        ['order_status' => 1],
                        ['id' => $v['order_id']]
                    );
                    if (!$res) exit(json_encode(['status' => 0, 'msg' => 'id为' . $v['order_id'] . '确认收款失败']));
                } else {
                    if ($order_status['purchase'] == 1) {
                        $record_integral = pdo_insert('ewei_users_integral', [
                            'integralnum' => $v['integral'],
                            'type'        => 1,
                            'mid'         => $v['store']['uid'],
                        ...
                        $update_user = pdo_update('users',
                            ['integral' => $v['user_integral'] + $v['integral']],
                            ['uid' => $v['store']['uid']
                            ]);
                    }
                    $res = pdo_update('ewei_shop_purchase_order',
                    ...
                    if (!$res || !$purchase_res) exit(json_encode(['status' => 0, 'msg' => 'id为' . $v['order_id'] . '确认发货失败']));
                }
            }
