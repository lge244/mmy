...
class Index_EweiShopV2Page extends WebPage
{
	...
	{
		global $_GPC;
		$service_list = pdo_getall('users', ['jobid' => 2]);
		$m_order = pdo_getall('ewei_shop_order', array('status' => 3));
		$count = 0;
		foreach ($m_order as $k => $v) {
			$count += $v['price'] - $v['dispatchprice'] - $v['taxes'];
		}
		$sum_service_ratio = pdo_getcolumn('service', [], 'sum_service_ratio');
		if ($_SERVER['REQUEST_METHOD'] == 'POST') {
			$uid = $_GPC['uid'];
			if (empty($uid)) $this->message('參數錯誤');
			$res = pdo_update('users', ['brokerage +=' => number_format($count * $sum_service_ratio / 100, 2)], ['uid' => $uid]);
			if (!$res) $this->message('分紅失敗');
			$this->message('分紅成功');
		}
		include $this->template();
	}
