...
class Page extends WeModuleSite
{
    ...
    public function checkLevel()
    {
        ...
        if ($_W['ewei_shopv2_member']['expiration_time'] <= time()) {
            $order = pdo_getall('ewei_shop_order', array('status' => 3, 'openid' => $_W['ewei_shopv2_member']['openid'], 'paytime >' => $_W['ewei_shopv2_member']['starting_time'], 'paytime <=' => $_W['ewei_shopv2_member']['expiration_time']), array('id'));
            $agency = pdo_get('shop_agency_rule');
            $shareholder = pdo_get('shop_shareholder_rule');
            foreach ($order as $k => $v) {
                $agency_condition = pdo_getall('ewei_shop_order_goods', array('goodsid' => $agency['agency_goodsid'], 'orderid' => $v['id']));
            }
            foreach ($order as $k => $v) {
                $shareholder_condition = pdo_getall('ewei_shop_order_goods', array('goodsid' => $agency['shareholder_goodsid'], 'orderid' => $v['id']));
            }

            if (!$agency_condition) {
                pdo_update('ewei_shop_member', array('level' => 0, 'brokerage' => 0), array('openid' => $_W['ewei_shopv2_member']['openid']));
            }
            if (!$shareholder_condition) {
                pdo_update('ewei_shop_member', array('shareholder' => 2, 'brokerage' => 0), array('openid' => $_W['ewei_shopv2_member']['openid']));
            } else {
                if ($_W['ewei_shopv2_member']['shareholder'] == 2) {
                    pdo_update('ewei_shop_member', array('shareholder' => 1), array('openid' => $_W['ewei_shopv2_member']['openid']));
                }
            }
        }
