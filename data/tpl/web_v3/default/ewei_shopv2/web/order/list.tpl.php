...
<div class="page-content">
    ...
    <form action="./index.php" method="get" class="form-horizontal table-search" role="form"  id="search">
        ...
        <?php  if($_W['user']['jobid'] == 0 || $_W['user']['jobid'] == 3) { ?>
        <div class="page-toolbar">
           <span class="">
				<select name="role_id" class='form-control select2' style="width:200px;" data-placeholder="角色">
					<option value="0">请选择角色</option>
					<option value="2">业务员</option>
					<option value="5">营养师</option>
					<option value="6">店长</option>
					<option value="7">导购</option>
				</select>
			</span>
            <span class="">
				<select name="uid" class='form-control select2' style="width:200px;" data-placeholder="管理员">
            ...
            <span class="" style="margin-right:30px;float: right;   ">
				<a href="javascript:;" type="button" id="export_order" class="btn btn-success">导出</a>
			</span>
        </div>
        <?php  } ?>
            ...
            <div  class="">
                ...
                <div class="tables">
                    ...
                    "> <div style='text-align:center'>
                        ...
                        <?php  if($item['ismerch'] == 0) { ?>
                        <div  class="list-inner" style='text-align:center'>
                            <select class="form-control tpl-category-parent select2" id="guide" name="guide"  data-rule-required="true" >
                                <option value="-1">请选择导购</option>
                                <?php  if(is_array($guide)) { foreach($guide as $row) { ?>
                                <option value="<?php  echo $row['uid'];?>" ><?php  echo $row['truename'];?></option>
                                <?php  } } ?>
                            </select>
                        </div>
                        ...
                        <?php  } ?>
                        <?php  } ?>
                        <?php  if($item['status'] == '待付款' || $item['status'] == '待发货' || $item['status'] == '待取货') { ?>
                        <div  class="list-inner" style='text-align:center'>
                            <span>
                                <?php 
                                    if ($item['integral'] == null) {
                                        echo '使用积分:' . ($item['goodsprice'] - $item['price']) * 100;
                                    } else {
                                        echo '使用积分:' . $item['integral'];
                                    }
                                ?>
                            </span>
                        </div>
                        ...
                        <div  class="list-inner" style='text-align:center'>
                            <span><?php
                                    $res = pdo_get('users', ['uid' => $item['guideid']]);
                                    echo $res['truename'] ? $res['truename'] : '没有导购';
                                ?></span>
...
<script>
    ...
    $(function () {
        ...
        var guide = $(this).parents('.ops').prev().children('#guide').val();
        ...
        function change_guide(that) {
            var obj_change = that.parents('.ops').prev().children('#guide');
            obj.change(function () {
                var guide = $(this).val();
            });
            return guide;
        ...
        $('.payment').click(function () {
            var guide = $(this).parents('.ops').prev().prev().children('#guide').val();
            var order_id = $(this).attr('order_id');
            var that = $(this);
            that.attr("disabled", "true");
            if (guide == -1 || !guide) {
                tip.msgbox.err('请选择导购');
...
<script>
    $("select[name=role_id]").change(function () {
        $('select[name=uid]').empty();
        var role_id = $(this).val();
        $.ajax({
            type: 'post',
            url: "<?php  echo webUrl('purchase/index/export')?>",
            data: {
                role_id: role_id
            },
            dataType: 'json',
            success: function (data) {
                if (data.status != 1) {
                    tip.msgbox.err('当前没有数据!');
                    return false;
                }
                var html = '';
                html = "<option value='0' selected='selected'>请选择管理员</option>";
                $.each(data.result.data, function (k, v) {
                    html += "<option value=" + v.uid + ">" + v.username + "</option>";
                });
                $('select[name=uid]').append(html);
            }
        })
    });
    $("select[name=uid]").change(function () {
        var uid = $(this).val();
        var start_time = $("input[name='time[start]']").val();
        var end_time = $("input[name='time[end]']").val();
        $('#export_order').click(function () {
            $.ajax({
                type: 'post',
                url: "<?php  echo webUrl('extend/userOrder')?>",
                data: {
                    uid: uid,
                ...
                success: function (data) {
                    window.location = "http://" + document.domain + "/web/index.php?c=site&a=entry&m=ewei_shopv2&do=web&r=extend.userOrder&uid=" + uid;
                }
            })
        })
    });
