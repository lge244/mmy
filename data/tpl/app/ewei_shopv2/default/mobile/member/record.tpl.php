    ...
    <div class='fui-content member-page navbar'>
        <div class="list">
            ...
            <ul class="content">
                ...
                <li class="operate"><?php  if(($row['type'] == 0)) { ?> 佣金增加 <?php  } else { ?> 佣金减少 <?php  } ?></li>
                <li class="user"><?php  echo date('Y-m-d',$row['time'])?></li>
            </ul>
            <?php  } } ?>
            <?php  if(is_array($list2)) { foreach($list2 as $row) { ?>
            <ul class="content">
                <li class="date">每月系统产生</li>
                <li class="number"><?php  echo $row['money'];?></li>
                <li class="operate"><?php  if(($row['type'] == 0)) { ?> 分红增加 <?php  } else { ?> 分红减少 <?php  } ?></li>
                <li class="user"><?php  echo date('Y-m-d',$row['time'])?></li>
            </ul>
            <?php  } } ?>
        </div>
